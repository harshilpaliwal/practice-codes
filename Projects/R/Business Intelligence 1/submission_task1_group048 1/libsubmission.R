...
require("zip") # add to top of task1.R
...
output_logfile <<- "logged_output.txt"
...
submission_includefiles <<- c(output_logfile,"flattable.RData","flattable.csv","casesperson.RData","casesmedrec.RData","casestravel.RData","geomapping.RData",workspace_filename,everywhere_includefiles)
...
libsubmission_versioninfo <- function() {
  patches <- paste(
    "2020-06-02 Initial version",
    "2020-06-01 Replaced zip()-function by the one provided by library('zip'); added logging functionality; fixing missing inclusion of some files in zip archive. ",
    "2020-06-07 15:25 bugfix for closing of logged_output.txt in submission_finalise(), added stop_logging() to quietly close output diversion and to close the log files; added version info.", sep="\n")
  cat("This version of libsubmission was built on 2020-06-07 15:13. Included patches:")
  cat("\n----------------------------\n")
  cat(patches)
  cat("\n----------------------------\n")
}
libsubmission_versioninfo()
...
save_answer <- function(quid,answer,is_file=FALSE,ignore_format=FALSE){
  ...
  if (check_result$success) {
    ...
    assert_that(is.null(try(saveRDS(answerlist, file = answerlist_filename, ascii = TRUE, version="3"))),msg=msg_filepermission)
    local({
    ...
    if (answerspecs[[thisanswer$quid]]$type!="file") {
      ...
      assert_that(is.null(try(saveRDS(thisanswer$answer, file = thisanswer$filename, ascii = TRUE, version="3"))),msg=msg_filepermission)
      local({
...
submission_finalise <- function(prefix="submission", additional_files_to_include = c()){
  ...
  stop_logging() # must be before checking for all files and packing them in the ZIP-compressed archive
  closeAllConnections() # to be sure 
...
close_logging <- function() {
  # Closes all sinks and closes the output_logfile
  while (sink.number()>0) {
    # there is a sink for output diversion already running!
    # this is closed before starting a new one later
    sink()
  }
  if (sink.number(type = "message")!=2) {
    # there is a sink for error/warning message diversion already running!
    # this is closed before starting a new one later
    sink(type = "message")
  }
  # Close all connections to the output_logfile:
  con_tab = getAllConnections()
  if (!is.null(dim(con_tab))) {
    for (logging_con_idx in which(con_tab[,1]==output_logfile)) {
      # Close the connection to that file
      print(paste("Closing connection <",row.names(con_tab)[logging_con_idx],"> to file <",con_tab[logging_con_idx,1],">",sep=""))
      close(getConnection(logging_con_idx))
    }
  }
}
start_logging <- function() {
  ...
  message(paste("Note: From now on, the console output is diverted to the file <",output_logfile,"> for logging. It will be shown after submission_finalise() is called. To stop this and to show the output, call stop_logging().",sep=""))
  logging_file <<- file(output_logfile, open = "wt")
  sink(file = logging_file, type = "message")
  sink(file = logging_file, type = "output", split = TRUE)
  logging_on <<- TRUE
}

stop_logging <- function(force=FALSE) {
  if (force==TRUE) stop_it = TRUE
  else {
    stop_it = FALSE
    if (exists("logging_on")) {
      if (logging_on==TRUE) {
        stop_it = TRUE
      } else warning("Logging seems not to be active. To force deactivation, call stop_logging(force=TRUE)")
    } else warning("Logging seems not to be active. To force deactivation, call stop_logging(force=TRUE)")
  }
  if (stop_it==TRUE) {
    ## revert output back to the console -- only then access the file!
    close_logging()
    file.show(output_logfile)
    message(paste("Note: Logging of the console output has been stopped. The logged content has been recorded in the file <",output_logfile,">.",sep=""))
    logging_on <<- FALSE
  }
}  
