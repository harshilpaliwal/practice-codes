...
save_answer <- function(quid,answer,is_file=FALSE,ignore_format=FALSE){
  ...
  if (check_result$success) {
    ...
    assert_that(is.null(try(saveRDS(answerlist, file = answerlist_filename, version="3"))),msg=msg_filepermission)
    local({
    ...
    if (answerspecs[[thisanswer$quid]]$type!="file") {
      ...
      assert_that(is.null(try(saveRDS(thisanswer$answer, file = thisanswer$filename, version="3"))),msg=msg_filepermission)
      local({
